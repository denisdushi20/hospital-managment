  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-12 text-center bg-gray-50">
        <h1 className="text-4xl font-bold text-yellow-600">SSG Produkti #{product.id}</h1>
        <h2 className="text-2xl mt-6 font-semibold text-blue-800">{product.name}</h2>
        <p className="mt-4 text-gray-700">{product.description}</p>
        <p className="mt-2 text-lg font-semibold text-yellow-700">Çmimi: €{product.price}</p>
        <p className="mt-4 text-sm text-gray-500">Renderuar me Static Site Generation.</p>
      </main>
      <Footer />
    </div>
  );
