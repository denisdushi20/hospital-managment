...
export default function DoctorAddModal({
  isOpen,
  onClose,
  onAdd,
}: DoctorAddModalProps) {
  // Initial state for the form fields for a NEW doctor
  ...
  const handleChange = (
    ...
    if (name.startsWith("address.")) {
      const addressField = name.split(".")[1] as keyof NewDoctorData["address"];
      setFormData((prev) => ({
        ...prev,
        address: {
          ...prev.address,
      ...
      setFormData((prev) => ({
        ...prev,
        [name as keyof NewDoctorData]: value,
      }));
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-40 p-4 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg mx-auto flex flex-col max-h-[90vh]">
        ...
        <form
          id="doctorAddForm" // Unique ID for this form
        ...
        <div className="flex justify-end space-x-3 px-6 py-4 border-t border-gray-200 mt-auto flex-shrink-0">
          ...
          <button
            type="submit"
            form="doctorAddForm" // Link this button to the form with id "doctorAddForm"
